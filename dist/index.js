...
async function runTradingAgents(payload, timeoutSeconds) {
...
export default defineToolPlugin({
    ...
    tools: (tool) => [
        tool({
            ...
            execute: async ({ timeoutSeconds = 120 }) => runTradingAgents({
                action: "health",
            }, timeoutSeconds),
        }),
        tool({
            name: "tradingagents_analyze",
            description: "Run TradingAgents analysis for a market ticker and return a concise decision report. Use for stock, ETF, index-proxy, Hong Kong, China A-share, India, crypto, and other Yahoo Finance tickers.",
            parameters: Type.Object({
                ticker: Type.String({
            ...
            execute: async ({ ticker, analysisDate, llmProvider, deepThinkLlm, quickThinkLlm, backendUrl, outputLanguage, maxDebateRounds, maxRiskRounds, checkpoint, timeoutSeconds = 1800, }) => runTradingAgents({
                action: "analyze",
                ticker,
                analysisDate,
                llmProvider,
